...
We evaluate rule discovery under a fixed split and a shared request representation. The local CSIC 2010 copy provides the labeled HTTP requests, while the harness exposes raw, decoded, and parsed fields from each row. We use the same stratified 80/20 split for the rule detector and the reproduced machine-learning baseline. This setup keeps differences in performance attributable to the detector design rather than to different data, preprocessing, or test partitions.
...
The rule-based detector is evaluated separately from the machine-learning baseline. It parses each request into raw and decoded fields, compiles a fixed set of Python regexes, and applies both regex rules and deterministic endpoint heuristics. The final rule set contains 22 regex rules and 12 callback heuristics. We evaluate these frozen rules on the same stratified 80/20 split as the random-forest baseline, using random seed 42; Table~\ref{tab:experimental-configs} summarizes the two detector configurations. The rule families were developed through iterative error analysis before this split-based reporting pass, so the result should be read as a held-out evaluation of a fixed detector rather than as a fully automated train-only discovery study.
...
The frozen rule set recovers nearly all of the benchmark structure while remaining compact and fast. We first report its held-out classification performance, then compare it with the reproduced machine-learning baseline on the same split. We then separate runtime from accuracy so that deployment cost is not conflated with discovery-time LLM use. Finally, we examine the remaining errors to show which attack patterns are still difficult for simple rules.
...
The reproduced baseline also shows that feature choice matters. Numeric-only models perform substantially worse than the TF-IDF random forest, with the numeric-only random forest reaching 0.8931 accuracy and 0.8691 F1. Gradient boosting and logistic regression over the same numeric features perform lower still. This reinforces the same observation that motivates the rule harness: most of the useful evidence is in request text and request structure, not in coarse scalar summaries alone. Table~\ref{tab:rule-iterations} shows the same pattern within the rule harness, where endpoint-aware rule groups produce the largest gains over seed signatures.

\subsection{Runtime}

The regex detector has a much smaller local pipeline cost than the reproduced machine-learning baseline. Applying the final rule set to all 61{,}065 requests took about 1.92 seconds after request construction, and the full load-parse-evaluate run took about 2.78 seconds. This corresponds to roughly 21{,}963 requests per second end to end on the local machine. The reproduced notebook reported 29.87 seconds of script runtime and 63.52 seconds of earlier shell wall-clock time for feature extraction, training, evaluation, and plot generation, as summarized in Table~\ref{tab:runtime}. These numbers are not identical deployment workloads, but they show the practical difference between a compiled rule set and a full model-training notebook pipeline.
...
LLMs can help write detection rules without becoming the detector. This paper built a harness that uses an LLM to propose regexes and endpoint heuristics, then relies on deterministic evaluation to decide which rules to retain. On the same stratified 80/20 split used by the reproduced random-forest baseline, the frozen rule set reached high held-out accuracy and ran over the full dataset in seconds. The result suggests that much of CSIC 2010's structure can be captured by auditable rules when generic attack signatures are combined with endpoint-specific invariants. More broadly, the study points to a practical division of labor: use models to search the rule space offline, but enforce precision, recall, and deployment behavior through a compact deterministic harness. Future work should freeze the split before discovery begins and repeat the study across additional web-attack datasets.
...
The accompanying GitHub repository is available at \url{https://github.com/RahulTripathi0401/239AS-final-project}. It contains the CSIC 2010 CSV used in the experiments, the deterministic rule harness, the agent-loop harness, the split-based evaluation script, the reproduced Kaggle notebook runner, and a reproducibility README with exact commands. The repository README explains how to regenerate the rule results, replay the agent loop, run the OpenAI planner, reproduce the machine-learning baseline, and rebuild this paper. All scripts use \path{project/csic_database.csv} and the same stratified 80/20 split with random seed 42.
